...
MCoef FCmatrixAlgo::Invert(MCoef& M){
	if (M.rows() != M.cols()){
		cerr << "Invert: not a square matrix" << endl;
		return {};
	}
	MCoef inv(MCoef::Identity(M.rows(), M.cols()));
	MOrder index_order(M.rows());
    ...
    for(int i=0; i<M.rows()-1; ++i){    	// Iterate from the first row to the penultimate row
	    ...
	    for(int j=i+1; j<M.cols(); j++){  // Forward elimination
		    double factor = M(j, i)/M(i, i);
		    M.row(j) -= factor*M.row(i);
		    inv.row(j) -= factor* inv.row(i);
	    }
    }
    // Backward substitution
    for (int k = M.rows() - 1; k >= 0; k--){
	    inv.row(k) /= M(k, k);    // normalize pivot
	    for (int i = k - 1; i >= 0; i--) {
		    inv.row(i) -= M(i, k) * inv.row(k);   // eliminate k column
	    }
    }
